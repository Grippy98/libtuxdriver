...
if __name__ == "__main__":
    setup()
    try:
        print("Tux is ready to speak! Type 'exit' or 'quit' to stop.")
        while True:
            try:
                # Support both Python 2 and 3 input
                try:
                    text = raw_input("What should Tux say? > ")
                except NameError:
                    text = input("What should Tux say? > ")
                ...
                if audio_file:
                    speak(audio_file)
                    # Clean up the file immediately so we don't accumulate them or get conflicts
